...
public class FlwHisTaskActorDaoImpl implements FlwHisTaskActorDao {
    ...
    @Override
    public List<FlwHisTaskActor> selectCcTaskActorsByInstanceId(String instanceId) {
        QueryWrapper wrapper = QueryWrapper.create()
                .from(FlwHisTaskActor.class)
                .where(FlwHisTaskActor::getInstanceId).eq(instanceId)
                .and(FlwHisTaskActor::getWeight).eq(6);
        return this.hisTaskActorMapper.selectListByQuery(wrapper);
    }
