...
import io.github.hicetop.bpm.engine.ProcessService;
import io.github.hicetop.bpm.engine.TaskAccessStrategy;
...
public class TaskServiceImpl implements TaskService {
     ...
     */
    @Override
    public boolean forceCompleteAllTask(String instanceId, FlowCreator flowCreator, InstanceState instanceState, TaskEventType eventType) {
     ...
     */
    @Override
    public Optional<FlwTask> executeJumpTask(String taskId, String nodeKey, FlowCreator flowCreator, Map<String, Object> args,
     ...
     */
    @Override
    public boolean assigneeTask(String taskId, TaskType taskType, FlowCreator flowCreator, List<FlowCreator> assigneeFlowCreators, Function<FlwTask, Boolean> check) {
        // 受理任务权限验证
     ...
     */
    @Override
    public FlwTask resume(String taskId, FlowCreator flowCreator) {
        ...
        // 流程实例结束情况恢复流程实例
        final String instanceId = histTask.getInstanceId();
    ...
    public Optional<FlwTask> withdrawTask(String taskId, FlowCreator flowCreator) {
        return this.undoHisTask(taskId, flowCreator, TaskType.withdraw, hisTask -> {
            ...
            if (performType == PerformType.countersign) {
                ...
                List<String> hisTaskIds = hisTaskDao.selectListByInstanceIdAndTaskNameAndParentTaskId(hisTask.getInstanceId(),
            ...
            // 查询任务参与者
            List<String> taskActorIds = taskActorDao.selectListByTaskIds(taskIds)
     ...
     * @return 任务参与者
     */
    protected Optional<FlwTask> undoHisTask(String hisTaskId, FlowCreator flowCreator, TaskType taskType,
    ...
    public List<FlwTask> createTask(NodeModel nodeModel, Execution execution, Function<FlwTask, FlwTask> taskFunction) {
        ...
        if (TaskType.major.eq(nodeType)) {
            ...
            if (Objects.equals(2, callProcessArr.length)) {
                flwProcess = processService.getProcessById(callProcessArr[0]);
            } else {
     ...
     */
    @Override
    public boolean addTaskActor(String taskId, PerformType performType, List<FlwTaskActor> flwTaskActors, FlowCreator flowCreator) {
        FlwTask flwTask = taskDao.selectCheckById(taskId);
